...
When you update your application code, the workflow has several steps:

1. Copy your new code into the rootfs directory.
2. Rebuild the filesystem image.
3. Compute a new hash tree and root hash.
4. Update your initramfs init script with the new root hash.
5. Rebuild the initramfs archive.
6. Regenerate your IGVM file and compute the new expected measurement.
