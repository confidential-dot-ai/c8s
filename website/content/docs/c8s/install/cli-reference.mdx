...
| `--operator-keys` | string | `""` | No | path to a PEM bundle of operator EC public keys that authorize [`c8s allowlist`](#c8s-allowlist) writes; sets `cds.operatorKeys`. Without it, allowlist writes are disabled (reads still served). See [creating the operator credential](/docs/c8s/attestation/allowlist#create-the-operator-credential) |
