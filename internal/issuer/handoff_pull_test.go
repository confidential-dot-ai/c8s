...
func pullTestHandoffServer(t *testing.T, tokenKey *ecdsa.PrivateKey, allowed map[string]bool) (*httptest.Server, *CA, *tls.Certificate) {
	t.Helper()
...
func TestPullHandoffRetriesTransientThenSucceeds(t *testing.T) {
	tokenKey := handoffTestKey(t)
	allowed := map[string]bool{"allowed_measurement": true}
	srv, _, clusterIdentity := pullTestHandoffServer(t, tokenKey, allowed)
...
func TestPullHandoffFailsFastOnDefinitiveVerdict(t *testing.T) {
	...
	srv, _, clusterIdentity := pullTestHandoffServer(t, tokenKey, map[string]bool{"other_measurement": true})
