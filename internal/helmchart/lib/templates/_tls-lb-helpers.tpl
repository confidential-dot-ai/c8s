...
CDS mesh-cert identity (see c8s-lib.tlsLb.san); the whole list is joined into nginx
server_name by tls-lb-configmap.yaml. Fails if san is set but not a list.
*/}}
{{- define "c8s-lib.tlsLb.sanList" -}}
...
{{- toJson $san -}}
{{- else -}}
{{- toJson (list (printf "%s.%s.svc" (include "c8s-lib.tlsLb.fullname" .) .Release.Namespace)) -}}
{{- end -}}
{{- end -}}

{{/*
The single identity baked into the CDS-issued mesh cert (get-cert) and validated
by cds.dnsSanPatterns: the first entry of c8s-lib.tlsLb.sanList. Extra san entries
widen only nginx server_name, not the mesh cert.
*/}}
{{- define "c8s-lib.tlsLb.san" -}}
{{- first (include "c8s-lib.tlsLb.sanList" . | fromJsonArray) -}}
{{- end -}}

{{/*
Common labels.
*/}}
{{- define "c8s-lib.tlsLb.labels" -}}
helm.sh/chart: tls-lb-0.5.0
{{ include "c8s-lib.tlsLb.selectorLabels" . }}
...
*/}}
{{- define "c8s-lib.tlsLb.validateSan" -}}
...
*/}}
{{- define "c8s-lib.tlsLb.validateProtocol" -}}
...
*/}}
{{- define "c8s-lib.tlsLb.serverNameFromAddress" -}}
...
*/}}
{{- define "c8s-lib.tlsLb.validateProxyTLS" -}}
...
{{/*
c8s-lib.tlsLb.requireSecuredBackend fails the render on a proxied backend hop that is
...
may be "").
*/}}
{{- define "c8s-lib.tlsLb.requireSecuredBackend" -}}
{{- $tls := default dict .tls -}}
...
*/}}
{{- define "c8s-lib.tlsLb.proxySSLDirectives" -}}
{{- if eq .protocol "https" -}}
{{- $tls := default dict .tls -}}
...
*/}}
{{- define "c8s-lib.tlsLb.renderAllowlistRoute" -}}
...
{{- end -}}
{{- and .Values.tlsLb.allowlist.enabled (ne (include "c8s-lib.tlsLb.hasExplicitAllowlistRoute" .) "true") -}}
{{- end -}}
...
*/}}
{{- define "c8s-lib.tlsLb.hasExplicitAllowlistRoute" -}}
{{- $found := false -}}
...
*/}}
{{- define "c8s-lib.tlsLb.allowlistLocation" -}}
{{- $root := .root -}}
location{{ if .exact }} ={{ end }} {{ .path }} {
    {{- if default false $root.Values.tlsLb.cors.enabled }}
    {{- include "c8s-lib.tlsLb.corsLocationDirectives" $root.Values.tlsLb.cors | nindent 4 }}
    {{- else if eq (include "c8s-lib.tlsLb.protocolCorsEnabled" $root) "true" }}
    {{- include "c8s-lib.tlsLb.protocolCorsLocationDirectives" $root | nindent 4 }}
    {{- end }}
...
*/}}
{{- define "c8s-lib.tlsLb.validateCORS" -}}
{{- $cors := default dict . -}}
...
*/}}
{{- define "c8s-lib.tlsLb.validateRouteCORS" -}}
{{- if .cors -}}
{{- $cors := .cors -}}
{{- range $k, $_ := $cors -}}
{{- if ne $k "enabled" -}}
...
*/}}
{{- define "c8s-lib.tlsLb.corsLocationDirectives" -}}
{{- $cors := default dict . -}}
{{- $methods := join ", " (default (list "GET" "POST" "OPTIONS") $cors.allowMethods) -}}
{{- $headers := join ", " (default (list "Authorization" "Content-Type" "X-C8s-Session") $cors.allowHeaders) -}}
...
*/}}
{{- define "c8s-lib.tlsLb.protocolCorsEnabled" -}}
{{- $cors := default dict .Values.tlsLb.cors -}}
{{- $pe := true -}}
...
block.
*/}}
{{- define "c8s-lib.tlsLb.protocolCorsLocationDirectives" -}}
proxy_hide_header Access-Control-Allow-Origin;
proxy_hide_header Access-Control-Allow-Methods;
proxy_hide_header Access-Control-Allow-Headers;
proxy_hide_header Access-Control-Allow-Credentials;
...
Selector labels.
*/}}
{{- define "c8s-lib.tlsLb.selectorLabels" -}}
app.kubernetes.io/name: {{ include "c8s-lib.tlsLb.name" . }}
...
carry one; acme renders only on the node-image shape — it needs a TEE-held key
and a reachable :80 challenge (the kata guest exempts only tcp:8443 from the
inbound mesh redirect, so no acme on the pod shape), with HTTP-01-issuable
sanList entries.
*/}}
{{- define "c8s-lib.tlsLb.publicTLSMode" -}}
...
{{- fail "VALIDATION_ERROR kind=tlslb_acme_kata_port: tlsLb.publicTLS.mode=acme cannot render on the pod shape: the guest exempts only tcp:8443 from the inbound mesh redirect (C8S_MESH_INBOUND_PASSTHROUGH), so the HTTP-01 challenge on :80 never reaches nginx. Use the node-image shape" -}}
{{- end -}}
{{- if .Values.attestationApi -}}
{{- fail "VALIDATION_ERROR kind=tlslb_acme_runtime: tlsLb.publicTLS.mode=acme requires the node-image shape, where the ACME account and serving keys are TEE-held" -}}
{{- end -}}
{{- range $s := (include "c8s-lib.tlsLb.sanList" . | fromJsonArray) -}}
{{- if contains "*" $s -}}
...
*/}}
{{- define "c8s-lib.tlsLb.acmeCertDir" -}}/etc/c8s-acme-tls{{- end -}}
{{- define "c8s-lib.tlsLb.acmeChallengePort" -}}8402{{- end -}}
{{- define "c8s-lib.tlsLb.acmeHTTPPort" -}}8080{{- end -}}
...
for c8s-lib.getCertContainers' extraArgs. tls-lb owns its own cert provisioning,
so it adds discovery output and verbose logging to the shared get-cert flow.
*/}}
{{- define "c8s-lib.tlsLb.getCertCommonArgs" -}}
{{- if .Values.tlsLb.discovery.enabled }}
- --discovery-out={{ include "c8s-lib.tlsLb.discoveryFilePath" . }}
- --discovery-cds-cert-url={{ .Values.tlsLb.discovery.cdsCertPath }}
- --discovery-public-tls-mode={{ include "c8s-lib.tlsLb.publicTLSMode" . }}
...
the readiness gate is on outside the pod shape. Under kata the in-guest
policy-monitor serves the inventory on loopback, so nothing is mounted and
the pod needs no socket group either; on every node shape the directory
exists (chart installer or node-image baked).
*/}}
{{- define "c8s-lib.tlsLb.mountInventorySocket" -}}
{{- if and .Values.tlsLb.attest.expectedWorkload (ne (include "c8s-lib.kata" .) "true") -}}
true
{{- end -}}
{{- end -}}
...
c8s-lib.getCertContainers). Caller nindents into the Pod spec's initContainers
list.
*/}}
{{- define "c8s-lib.tlsLb.getCertContainers" -}}
{{- $mounts := list -}}
{{- if .Values.tlsLb.discovery.enabled -}}
{{- $mounts = append $mounts (printf "- name: discovery\n  mountPath: %s" .Values.tlsLb.discovery.mountPath) -}}
{{- end -}}
{{- with include "c8s-lib.attestationSocketMount" . -}}
{{- $mounts = append $mounts . -}}
{{- end -}}
{{- $extraArgs := include "c8s-lib.tlsLb.getCertCommonArgs" . | fromYamlArray -}}
...
{{- else if eq (include "c8s-lib.tlsLb.mountInventorySocket" .) "true" -}}
{{- $mounts = append $mounts "- name: workload-claims\n  mountPath: /run/c8s/workload-claims\n  readOnly: true" -}}
{{- end -}}
{{- end -}}
{{- if eq (include "c8s-lib.tlsLb.publicTLSMode" .) "webpki" -}}
{{- $mounts = append $mounts (printf "- name: public-tls\n  mountPath: %s\n  readOnly: true" .Values.tlsLb.publicTLS.mountPath) -}}
{{- $extraArgs = append $extraArgs (printf "--reload-watch=%s" (include "c8s-lib.tlsLb.publicCertPath" .)) -}}
{{- $extraArgs = append $extraArgs (printf "--reload-watch=%s" (include "c8s-lib.tlsLb.publicKeyPath" .)) -}}
{{- end -}}
{{- include "c8s-lib.getCertContainers" (dict
  "root" .
  "san" (include "c8s-lib.tlsLb.san" .)
...
{{/*
c8s-lib.tlsLb.meshWrappedUpstream — "true" when the address is an operator-managed
...
*/}}
{{- define "c8s-lib.tlsLb.meshWrappedUpstream" -}}
