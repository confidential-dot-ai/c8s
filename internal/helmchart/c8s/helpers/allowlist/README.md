...
`c8s.anyArgvDigests` extracts workload digests whose command, args, environment and mounts
policies are all `any`. `c8s.alwaysAllow` merges these with `c8s.imageAllowlist` for the
host plugin's local admission list. Workloads that pin any of those fields remain in the served
...
digest, with command, args and environment policies set to `any`, followed by
