...
func bindProxyFlags(fs *pflag.FlagSet, c *proxyConfig) {
	...
	fs.StringVar(&c.initData, "init-data", "", "hex launch-time init-data value (SNP HOST_DATA, 64 hex; TDX MRCONFIGID, 96 hex) mesh peers must carry — the launchdata commitment of the node ISO. Empty = no init-data pinning")
	fs.StringVar(&c.cdsInitData, "cds-init-data", "", "hex launch-time init-data value that CDS's RA-TLS peer cert must carry — the launchdata commitment of the CDS node's ISO. Empty = no init-data pinning")
...
func runProxy(ctx context.Context, c *proxyConfig) error {
	...
	meshPolicy.Policy.Images = peerPins.Images
	if meshPolicy.Policy.ExpectedInitDataHash, err = ratls.ParseHexInitData(c.initData); err != nil {
