...
func newMeshRuntime(cfg *ratls.ServerConfig, logger *slog.Logger, sessionCacheSize int) (*meshRuntime, error) {
	...
	serverCertMgr.SetOnRotationFail(func() { m.certRotationFailures.WithLabelValues("server").Inc() })
	if clientCertMgr != nil {
		clientCertMgr.SetOnRotationFail(func() { m.certRotationFailures.WithLabelValues("client").Inc() })
	}
	return &meshRuntime{
...
func (r *meshRuntime) run(ctx context.Context, env meshEnvironment) error {
	...
	p.onReady = func() { close(listenersReady) }
	rotations.Go(func() {
		select {
		case <-ctx.Done():
			return
		...
		rotations.Go(func() { r.serverCertMgr.RunRotation(ctx) })
		if r.clientCertMgr != nil {
			rotations.Go(func() { r.clientCertMgr.RunRotation(ctx) })
		}
