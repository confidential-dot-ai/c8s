...
func TestSandboxConfigValidation(t *testing.T) {
	tests := []struct {
		...
		yaml    string
		wantErr string
		want    string // expected effective policy.sandbox
	}{
		{
...
func TestSandboxRunsOnlyInTheFinalPhase(t *testing.T) {
	tests := []struct {
		...
		{name: "create phase admits", final: false, want: verdictAllow},
		{name: "final phase denies", final: true, want: verdictDeny},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			verdict, reason := p.checkContainerPhase(context.Background(), cfg,
				sandboxedPod(), privilegedContainer(), testFloorImage, tt.final)
			if verdict != tt.want {
				t.Fatalf("checkContainerPhase(final=%v) = %d %q, want %d", tt.final, verdict, reason, tt.want)
			}
		})
	}
}
