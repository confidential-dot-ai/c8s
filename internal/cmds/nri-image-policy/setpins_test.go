...
func TestSetCDSPinsWritesInitData(t *testing.T) {
	...
	if got := setPins(t, path, "--cds-measurements", pinA, "--cds-init-data", strings.ToUpper(hostData)); got != pinsUpdated {
		t.Fatalf("outcome = %q, want %q", got, pinsUpdated)
	}
	...
	if got := cfg.Allowlist.Pull.CDSInitData; got != hostData {
		t.Errorf("cds_init_data = %q, want %q", got, hostData)
	}
	if got := setPins(t, path, "--cds-measurements", pinA, "--cds-init-data", hostData); got != pinsUnchanged {
	...
	if cfg.Allowlist.Pull.CDSInitData != "" {
		t.Errorf("cds_init_data = %q, want empty", cfg.Allowlist.Pull.CDSInitData)
	}
}
