...
func TestSignLaunchWritesVerifiableSignature(t *testing.T) {
	...
	dir := t.TempDir()
	content := []byte("schemaVersion: c8s-launch/v2\nrole: leader\n")
...
func TestSignLaunchRefusesToOverwrite(t *testing.T) {
	...
	dir := t.TempDir()
	launchPath := writeLaunch(t, dir, []byte("schemaVersion: c8s-launch/v2\nrole: follower\n"))
...
func TestSignLaunchDetectsTamperedContent(t *testing.T) {
	...
	tamperedDigest := sha256.Sum256([]byte("schemaVersion: c8s-launch/v2\nrole: leader\n"))
