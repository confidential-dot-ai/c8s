...
rather than a transient runtime problem.`,
	...
	fs.BoolVar(&cfg.RequirePulledImageStamp, "require-pulled-image-stamp", false, "fail closed when a container bundle lacks the kata-agent-stamped pull reference (c8s-pulled-image). Set once the stamping kata-agent ships; without the stamp the decision falls back to host-authored OCI annotations, which are forgeable by the host")
	return cmd
}
