...
func cdsAllowlistHandler(t *testing.T, version string, digests map[string]string) http.HandlerFunc {
	t.Helper()
	body, err := anyAllowlist(t, digests).Canonical()
	if err != nil {
		t.Fatal(err)
	}
...
// A pull installs the served document as the overlay and leaves the baked seed
// exactly as loaded.
func TestRefreshOnce_InstallsServedDocument(t *testing.T) {
	...
	if !refreshOnce(context.Background(), testLogger(t), client, overlay, time.Second) {
...
func TestRefreshOnce_RolledBackVersionIgnored(t *testing.T) {
	...
	refreshOnce(context.Background(), testLogger(t), client, overlay, time.Second)
	high.Close()
	if overlay.version != 5 {
		t.Fatalf("overlay version = %d, want 5", overlay.version)
	}

	// A withheld/rolled-back CDS now serves version 3 with an extra entry.
