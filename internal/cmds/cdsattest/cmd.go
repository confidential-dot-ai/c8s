...
func NewCmd() *cobra.Command {
	...
	f.StringVar(&cfg.frontDoorMode, "front-door-mode", "", "REQUIRED: which credential terminates public TLS in front of this sidecar: cds (TEE-held mesh-issued serving key; attest-lb served), acme (TEE-held in-guest ACME serving key; attest-lb served), or webpki (host-visible Secret; attest-lb refused with unsupported_front_door)")
...
func run(cfg config) error {
	...
	case FrontDoorModeCDS, FrontDoorModeWebPKI, FrontDoorModeACME:
	default:
		return fmt.Errorf("--front-door-mode must be %q, %q, or %q, got %q", FrontDoorModeCDS, FrontDoorModeWebPKI, FrontDoorModeACME, cfg.frontDoorMode)
	}
