...
func (p LiveEvidenceProvider) Evidence(ctx context.Context, reportData []byte) (CollectedEvidence, error) {
	...
	return CollectedEvidence{Evidence: resp.Evidence, Platform: platform, Generation: generation,
		GPUAttested: normalizedGPUStatus(resp.GPUAttested), NvidiaGPU: resp.NvidiaGPU}, nil
}
...
func (p FixtureEvidenceProvider) Evidence(_ context.Context, _ []byte) (CollectedEvidence, error) {
	return CollectedEvidence{Evidence: p.Raw, Platform: p.Platform, Generation: p.Generation, GPUAttested: types.GPUAttestedUnknown}, nil
}
