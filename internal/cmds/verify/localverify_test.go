...
func TestVerifyEvidence_TimeoutBoundsCollateralFetch(t *testing.T) {
	...
	start := time.Now()
	code := verifyEvidence(context.Background(), cfg, &verifyPlan{policy: &ratls.VerifyPolicy{}}, bareSnpEvidence(t), nil, operatorKeysReport{}, measurementsReport{}, staticCAReport{}, &out, &errOut)
