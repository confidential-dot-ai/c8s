...
func NewCmd() *cobra.Command {
	...
	f.StringVar(&cfg.Image.ManifestPath, "image-manifest", "", "confos build manifest.json for the guest image the node must be running; pins tdx.mrtd, tdx.rtmr1 and tdx.rtmr2 together (oras pull ghcr.io/…/c8s-base:<tag>). REQUIRED unless the three are given individually — RTMR[3] is derived from your PUBLIC key and the host stages it at launch, so without an image pin any TDX guest can satisfy the gate")
