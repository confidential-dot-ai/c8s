...
func NewCmd() *cobra.Command {
	...
	flags.StringVar(&cfg.operatorKeys, "operator-keys", "", "Path to a PEM bundle of pinned operator EC public keys; operator tokens signed by one of them authorize allowlist mutations (unless --static-allowlist) and secret writes (empty = operator writes disabled, reads still served)")
	...
	flags.BoolVar(&cfg.staticAllowlist, "static-allowlist", false, "seal the allowlist: --allowlist-seed becomes the one immutable policy for this CDS's lifetime (the store is replaced with it, not merged), allowlist mutations stay disabled even when --operator-keys is set, while those keys can still authorize secret writes; the mesh CA certificate carries the document's canonical SHA-256 plus an RA-TLS attestation extension binding the CA key, so relying parties can pin the enforced policy (docs/static-allowlist.md)")
