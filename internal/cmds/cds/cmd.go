...
func NewCmd() *cobra.Command {
	...
	flags.StringVar(&cfg.nodeInitData, "node-init-data", "", "hex launch-time init-data value (SNP HOST_DATA / TDX MRCONFIGID) a node's evidence must carry to call /attest or serve a sandbox inventory — the launchdata commitment of the node ISO. Empty = no init-data pinning: any launch of a listed image is accepted")
