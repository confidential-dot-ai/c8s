...
func containersView(digests ...string) []workloadclaims.SandboxContainer {
	...
	for _, d := range digests {
		out = append(out, workloadclaims.SandboxContainer{Digest: d, MountsObserved: true})
	}
	return out
}
...
func TestAttest_MatchedWorkload_DropsInjectedContainers(t *testing.T) {
	...
	containers := []workloadclaims.SandboxContainer{
		{Digest: wlDigestA, MountsObserved: true},
...
func TestAttest_WorkloadStampRequiresMountEvidence(t *testing.T) {
	for _, tc := range []struct {
		...
		{"reviewed memory", []pkgallowlist.ObservedMount{{Destination: "/config", Class: pkgallowlist.MountData, Storage: pkgallowlist.MountMemory}}, true, true},
		{"unknown storage", []pkgallowlist.ObservedMount{{Destination: "/config", Class: pkgallowlist.MountData, Storage: pkgallowlist.MountUnknown}}, true, false},
		{"unobserved", nil, false, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			store := completeAPIStore(t)
			store.workloads["api"].Containers[0].Mounts = pkgallowlist.MountPolicy{Policy: pkgallowlist.PolicyExact, Destinations: []string{"/config"}, Reviews: map[string]string{"/config": "data only"}}
			containers := containersView(wlDigestA)
			containers[0].Mounts, containers[0].MountsObserved = tc.mounts, tc.observed
			matched := issueWithInventory(t, store, []string{wlDigestA}, containers, nil)
			if (matched != nil) != tc.want {
				t.Fatalf("stamp=%v", matched)
			}
