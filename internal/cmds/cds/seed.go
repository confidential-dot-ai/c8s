...
// into store, returning how many entries it added. It owns the file/wire-format
// concerns; the additive, version-stable merge is Store.SeedWorkloads.
//
...
func seedStore(store *allowlist.Store, path string) (int, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return 0, fmt.Errorf("read allowlist seed %q: %w", path, err)
	}

	seed, err := pkgallowlist.ParseJSON(data)
	if err != nil {
		return 0, fmt.Errorf("parse allowlist seed %q: %w", path, err)
	}

	added, err := store.SeedWorkloads(seed.Workloads)
	if err != nil {
		return 0, fmt.Errorf("seed allowlist workloads: %w", err)
	}
