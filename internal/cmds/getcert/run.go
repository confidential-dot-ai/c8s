...
alongside a workload that uses the obtained certificate.`,
	...
	flags.StringVar(&cfg.CDSInitData, "cds-init-data", "", "hex launch-time init-data value (SNP HOST_DATA / TDX MRCONFIGID) CDS's RA-TLS cert must carry — the launchdata commitment of the CDS node's ISO (empty = no init-data pinning)")
