...
func newHandler(cfg config, logger *slog.Logger) (http.Handler, error) {
	...
	httpClient, err := ratls.NewVerifyingHTTPClient(ratls.Pins{Measurements: measurements, RTMRs: rtmrs, PCRs: pcrs, InitDataHash: initDataHash, Entries: pinned.Entries}, cfg.attestationAPIURL)
	if err != nil {
