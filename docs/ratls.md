...
- **Which launch of an image, unless init-data is pinned.** A launch digest
  ...
  `cds --node-init-data` requires the pinned value of every `/attest` caller
  and every inventory it dials, and `--cds-init-data` (ratls-mesh, get-cert,
  allowlist-proxy; `allowlist.pull.cds_init_data` for nri-image-policy, set
  with `set-cds-pins --cds-init-data`) requires CDS's own launch to be the
  pinned one. `ratls-mesh --init-data` does the same for mesh peers. The pin
  travels to the attestation-api as `expected_init_data_hash` — 32 bytes on
  SNP, 48 on TDX — and a verdict that does not affirm the match is a refusal.
  Left empty, the image pins alone accept any launch of a listed image.
