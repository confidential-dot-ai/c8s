...
Certificates live 24h by default. ratls-mesh runs a timer for each server and
client certificate, renewing at 50% of TTL even while idle. Failed renewals
retry with exponential jittered backoff (5s initial interval, 1 minute maximum
base interval), including while expired and NotReady. Provider upgrades
reschedule the timer. Each attempt is bounded by the rotation timeout.
...
`cert_role`, and `ratls_mesh_cert_rotation_failures_total` has a `role` label.
