...
A CDS-served document cannot express the marker at all. `nodeTCB` is a
reserved field on an entry's container, and both parse paths — `c8s allowlist
lint` and the enforcer's own pull — refuse a document carrying it
(`allowlist.ErrNodeTCBInDocument`). Node-TCB status is a property of a measured
boot config, never of a document.
