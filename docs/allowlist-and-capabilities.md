...
`allowlist.floor` (an allowlist document baked into its boot config,
chart-rendered from the chart's own component digests plus every
...
plugin's boot floor still admits it. To block a
