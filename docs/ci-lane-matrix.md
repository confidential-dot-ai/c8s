...
|---|---|
| node image | Built per c8s merge into `ghcr.io/confidential-dot-ai/node-guest-base` as `rke2-tdx[-cdi]-<c8s-short-sha>`. The pinned tuple (`image`, `rootPvc`, `cdiTag`, `c8sRef`, `mrtd`, `rtmr1`, `rtmr2`) lives in ConfigMap `tdx-rke2-image-refs` (ns `confai-images`), published from [`c8s-e2e/cluster-prep/tdx-rke2-image-refs.cm.yaml`](../c8s-e2e/cluster-prep/tdx-rke2-image-refs.cm.yaml). **Values are deliberately not repeated here**: this table went stale for three weeks and cost a red lane. To bump, see [cluster-prep/README.md](../c8s-e2e/cluster-prep/README.md) |
...
| c8s CLI | build at the ConfigMap's `c8sRef`, whatever it currently is: `git clone .../c8s.git && git checkout "$c8sRef" && go install ./cmd/c8s`. An unstamped build reports version `dev`, so **`--image-tag "$c8sRef"` is mandatory**. The ref is not a free choice: the image bakes an admission floor from its own build ref, so a mismatched CLI gets its own components denied |
| install | `c8s install --cvm-mode node --single-node --hardware-platform tdx --image-tag "$c8sRef" --operator-keys <pub> --measurements <mrtd> --resolve-digests=true -f <extra-values>` |
