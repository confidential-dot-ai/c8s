...
| LB attestation + session key are TEE-bound | `c8s cds-attest` sidecar | SNP report `report_data = SHA-384(x25519 \|\| mlkem768 \|\| nonce)` (default PQ; does not bind the mesh identity, §5 Addressable) or `SHA-384(serving_leaf_spki \|\| nonce)` (`pq=false`, no PQ tunnel) |
