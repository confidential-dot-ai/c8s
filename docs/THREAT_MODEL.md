...
| The **sandbox-ID pin** (`c8s verify --sandbox-id --mesh-ca`, `VerifyPolicy.SandboxID`) rests on the mesh CA signature and an honest inventory, not on hardware evidence. The ID is stamped into the leaf's signed area, not folded into REPORTDATA, so only a CA-chain check authenticates it — `VerifyCert`/`VerifyAttestation` fail closed on a pin, and the mesh enforces it solely on the CA-verified path. A workload cannot forge the binding (the sandbox token is signed by an inventory key CDS attested, and names the sandbox the kernel reported) and cannot dodge the image gate (CDS asks that inventory directly at issuance and requires every running image to be allowlisted; it does **not** require the set to match one workload entry, since issuance lands mid-lifecycle — see getcert-workload-binding.md Corner 4) — but a compromised **inventory** on an allowed measurement can vouch for a sandbox arbitrarily, and a compromised **mesh CA** can mint any ID. | compromised inventory / mesh CA | Per-workload measurement enforced at `/attest`, so the running image set is hardware-bound rather than inventory-asserted. Interim: treat the ID as CA-vouched identity, not attested identity. | `docs/getcert-workload-binding.md` (Corner 5/6); `docs/ratls.md` ("What vouches for the ID") |
...
| CDS's sandbox-digests callback dials an address the **requester** chose. A sandbox token is mintable by anything holding a CDS `/attest-key` EAR — on node-CVM every pod, since they share the node's launch measurement — so `inventoryAddr` is attacker-controlled, not inventory-controlled. Bounded rather than eliminated: `parseInventoryAddr` allows only routable unicast IP literals (no names, so DNS cannot redirect after the check; no loopback, link-local/IMDS, multicast, or unspecified), the callback is mutually-attested RA-TLS so nothing but an attested peer can be spoken to, and CDS returns a generic denial instead of the callback's outcome. Residual: a pod can still make CDS open a TLS connection to a routable address of its choosing and infer reachability from timing. | co-tenant pod on node-CVM | Constrain the callback to operator-configured node addresses (CIDR or a discovered node list) instead of a token-carried one. | `pkg/workloadclaims/digests.go` (`parseInventoryAddr`); `internal/cmds/cds/attest.go` |
