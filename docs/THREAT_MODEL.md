...
| Privileged cluster operator holding a pinned operator key | Trusted with that key | Can rewrite the image-integrity allowlist. Keys are long-lived; revocation is coarse. The **public** half is not a secret and the host stages it on the opkeydata disk at launch, so `RTMR[3] = SHA384(0x00*48 ‖ SHA384(pubkey))` is reproducible by the host on any guest image it chooses: RTMR[3] proves *whose deployment this is*, never *what software booted*. Any verifier gating on it MUST also pin the image (MRTD + RTMR[1] + RTMR[2]) or it authenticates nothing an attacker cannot replicate — `c8s get-kubeconfig` requires an `--image-manifest` for exactly this reason. |
