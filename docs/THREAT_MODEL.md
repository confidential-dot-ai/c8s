...
> **Release:** v0.1.0 · **Milestone:** operator-consolidation · **Last updated:** 2026-07-15 (v0.1.0).
...
|---|---|---|---|
| The secret-broker's release policy is a ConfigMap, its reusable OpenBao token/AppRole secret ID is a Kubernetes Secret, its image/arguments/trust roots are pod-spec inputs, and its workload identity is a control-plane-selected SAN. The control plane can therefore rewrite authorization, steal the backing-store credential, or impersonate an allowed workload. A node-as-CVM quote covers the shared node, not any one workload or runtime spec. | control plane / host | Release a short-lived credential only to an attested in-CVM key; bind signed policy, broker startup inputs, workload spec/identity, and that key to composed evidence. Interim: the chart refuses the broker unless a dev/test deployment explicitly trusts Kubernetes; this acknowledgement is not a runtime boundary. | decision 2026-07-15; GAPS §Trust model; #46, #49, #57, #305 |
