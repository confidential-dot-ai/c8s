...
> **Validated end-to-end 2026-07-22** on `integration/kms-all`, against a
> single-node RKE2 cluster inside a real Intel TDX guest (`--cvm-mode node
> --hardware-platform tdx`): every beat below (1–10, including the LUKS
> create → attested passphrase release → in-pod decrypt → ciphertext-at-rest
> → persistence chain) ran green. Prep steps P.4/P.5 and Parts 0/1/7 carry
> corrections from that run — if you are following an older copy of this file,
> re-read those.
...
- A CVM-capable cluster. This runbook assumes **Azure CVM** (`--cvm-mode
  ...
  identical.
- `kubectl`, `helm`, the `c8s` binary (this branch) and `crane` on the demo
  machine; root on one node for the LUKS `local` driver, plus the `c8s` binary
  on that node (Part 7 runs there). The `bao` CLI is convenient but **not
  required** — every store interaction below is a plain KV v2 HTTP call, so
  `curl -H "X-Vault-Token: root" …` against the Part 2 port-forward does the
  same job.
...
**Assume every digest above is stale.** They move on every branch build, and
`attestation-api:main` moves independently of c8s. Re-resolve before each run
rather than trusting the paste — a stale pin fails at pull time, mid-demo.

Longer term this should be fixed on the branch by marking
`attestationApi.image` as `externalImage: true` in `c8sComponents` (the same
treatment `secretAgent.image` has) so the CLI skips it for `--image-tag`
rewriting and digest resolution. Note that flag alone is **not** sufficient:
`secretAgent.image` also carries a hard-coded default `digest` in
`values.yaml`, and `attestationApi.image` has neither `tag` nor `digest` set,
so marking it external without also giving it a default digest just moves the
failure from `crane` to the chart's "tag or digest required" render guard.
Both changes together, or neither. Not blocking for the demo.
...
**`--single-node` is not optional on a demo cluster with no dedicated CDS
node**, and getting it wrong is the single worst failure in this runbook
because it is *silent*. The chart defaults `cds.node.selector` to
`{role: cds}`; if no node carries that label, `c8s-cds` sits `Pending`
forever with `FailedScheduling … didn't match Pod's node affinity/selector`.
The preflight that would catch this (`preflightCDSNode`) is deliberately
skipped whenever `-f` is supplied — and this runbook *always* supplies `-f` —
so you get no error at all: helm's `--wait` simply hangs for its full timeout
and leaves the release `STATUS: failed`. Use `--single-node` (clears the
selector and the dedicated-node toleration), or label a node `role=cds`
first. See P.7 for why a late CDS also wedges every other component.
...
Under `--cvm-mode node --hardware-platform tdx` (or `sev-snp`), **label the
TEE node first** — the install refuses otherwise:

```sh
kubectl label node <node> confidential.ai/tdx=true    # or confidential.ai/sev-snp=true
```

That one fails fast with a message naming the exact command, so it costs a
retry rather than a debugging session. A TDX node also needs `/dev/tdx_guest`,
`qgsd` running, and the QGS vsock bridge (see the c8s TDX host notes).
...
reachable); that one does recover on its own. Wait for `Running 2/2`:
...
> **If a pod is stuck in `Init:1/2`, it is not slow — it is wedged, and
> waiting will not fix it.** The injected `c8s-cert` retries CDS for
> `--initial-retry-timeout` (2m) and then, rather than crashing, falls back to
> its renewal loop: `--renew-interval=1h` for chart components, **6h** for
> workload pods. So if CDS is more than ~2 minutes late — which is exactly
> what a missing `--single-node` causes — every dependent pod burns its cert
> budget and then sits in `Init` for the next hour. Recovery is a pod delete,
> not patience:
>
> ```sh
...
> with `initial certificate request failed, will retry next interval` followed
> by `entering renewal loop`. This is fail-closed working as designed (no pod
> starts without a real mesh cert); it is only the *recovery latency* that
> bites during a demo.
...
### Part 0 — Port-forward CDS (keep it running)

CDS has no public ingress, so reach it through a forward. The allowlist CLI
verifies CDS's RA-TLS evidence **in-process** (same code path as `c8s cds
verify`) — it does *not* call the attestation-api, and has no
`--attestation-api-url` flag:
...
ALLOWLIST="--url https://localhost:8443"
...
This runs **on the node that will host the pod**, as root — the `local` driver
creates a loop-file on that host. Keep it small so the ciphertext checks stay
snappy.

Note the address change: Part 2's `127.0.0.1:8200` is a port-forward on your
*demo machine*, and the node has nothing listening there. Unless the demo
machine and the node are the same box, use the store's ClusterIP, which the
node can reach through kube-proxy:

```sh
# on the demo machine:
BAO_IP=$(kubectl -n c8s-system get svc c8s-openbao -o jsonpath='{.spec.clusterIP}')

# on the node (ssh in; the c8s binary must be present there too):
export BAO_ADDR=http://<BAO_IP>:8200
sudo -E c8s luks create \
  --workload api --name data --size 1Gi --mount /data \
  --openbao-addr $BAO_ADDR --openbao-token root
```

Parts 9 and 10 also run on the node and reuse this `BAO_ADDR`. (The
alternative is running `kubectl -n c8s-system port-forward svc/c8s-openbao
8200:8200` on the node itself, which keeps the literal `127.0.0.1:8200` in
every command — nice for a scripted demo, one more moving part on stage.)
...
# -q so the count doesn't print; the message is the whole point on stage
sudo grep -a -q kms-proof /var/lib/c8s/luks/api-data.img \
  && echo "LEAK: plaintext found on disk" \
  || echo "plaintext: not found (disk holds ciphertext only)"
```
...
# This also re-arms fail-closed: a NEW pod on that digest is denied again
# within the 30 s poll (already-running containers are untouched — the NRI
# plugin gates container *creation*).
c8s allowlist remove "$BUSYBOX_DIGEST" $ALLOWLIST
c8s allowlist list $ALLOWLIST | head

kill %1 %2   # the port-forwards (CDS, openbao)
```
...
- **Per-workload digest bind (`workloadImages`) — know its two limits before
  ...
  - **It does not bind through the Part 3 injection path.** The injected
    ...
    *next* cert renewal — first issuance is claim-free (getcert-workload-binding.md
    "Corner 4"). Verified on a live cluster: get-cert issued with
    `workload_claims=false` and the broker logged `workload_digest:"(none)"` on
    every login, **including granted ones**. Workload pods run
    `--renew-interval=6h`, so the rule would fail closed for ~6 hours and the
    app would never start. Demonstrate it instead against a pod that does not
    gate on the secret: `c8s verify --workload-image sha256:…` against that
    pod's mesh leaf (passes for the right digest, fails for any other), or a
    caller that fetches from the broker after startup.
  - **It is honest-workload detection, not identity.** CDS binds the claim to
    nothing the pod actually runs — it only checks that the forwarded list
    hashes to the evidence-bound digest and that every image is allowlisted.
    Any *allowlisted* workload can run the attest flow itself with
    caller-chosen REPORTDATA and assert another workload's image set, so the
    pin catches an honest workload drifting or a config swap — it does not stop
    a lying one from impersonating a victim. See THREAT_MODEL.md ("The
    workload-image pin distinguishes honest workloads only") and
    getcert-workload-binding.md Corners 5/6. Do not present this beat as
    "only this exact pod can ever read the secret".
...
| Symptom | Cause / fix |
|---|---|
| **`c8s-cds` stuck `Pending`; install hangs then reports `STATUS: failed`** | No node satisfies `cds.node.selector={role: cds}`. You almost certainly forgot `--single-node` (P.5). There is **no preflight error** for this — `preflightCDSNode` is skipped whenever `-f` is passed, and this runbook always passes `-f` — so the only symptom is the hang. `kubectl -n c8s-system describe pod -l app.kubernetes.io/component=cds` shows `FailedScheduling … didn't match Pod's node affinity/selector`. Re-run with `--single-node`, then delete any pod left in `Init` (next row). |
| **Pod stuck in `Init:1/2` long after CDS is healthy** | Its `c8s-cert` burned the 2 m `--initial-retry-timeout` while CDS was unavailable and fell back to `--renew-interval` (1 h components / 6 h workloads). It will **not** self-heal on a demo timescale. Confirm via `kubectl logs <pod> -c c8s-cert` (`initial certificate request failed…` → `entering renewal loop`), then `kubectl delete pod` it. |
| `unknown flag: --attestation-api-url` on `c8s allowlist` | That flag does not exist — the allowlist CLI verifies CDS's RA-TLS in-process. Use `--url https://localhost:8443` alone; only the CDS port-forward is needed for Part 1. |
