...
# The baked chart install (c8s-chart.yaml.in -> c8s-chart.yaml at build time,
# node-guest-image/c8s/mkosi.sync render_chart_helmchart) must never be
# createNamespace: true — c8s-namespace.yaml bakes c8s-system with the
# privileged PSA labels ahead of the HelmChart, following local-path-storage's
# precedent (baked namespace, not chart-managed), and a createNamespace: true
# HelmChart would create it WITHOUT those labels, letting restricted
# PodSecurity deny the chart's own privileged pods on first install.
chart_tmpl="$ngi/c8s/c8s-chart.yaml.in"
...
# mkosi.extra would ship literal @TOKEN@ placeholders no HelmChart controller
# can parse, and never get the resolved digests/chart content mkosi.sync
# substitutes.
if find "$ngi/c8s/mkosi.extra" -name 'c8s-chart.yaml.in' 2>/dev/null | grep -q .; then
  echo "::error::c8s-chart.yaml.in must not live under mkosi.extra — it is a template mkosi.sync renders, not a file to bake raw"
  exit 1
fi

# c8s-namespace.yaml (baked ahead of the chart) must carry the same three
# privileged PSA labels namespaceManifest() in cmd/c8s/install.go renders for
# a live install's release namespace — the chart's own privileged pods
# (nri-image-policy's baked installer pins, attestation-api, ratls-mesh) land
# in c8s-system before any operator-driven relabel could run.
...
for label in 'pod-security.kubernetes.io/enforce: privileged' \
             'pod-security.kubernetes.io/warn: privileged' \
             'pod-security.kubernetes.io/audit: privileged'; do
  if ! grep -qF "$label" "$c8s_ns"; then
    echo "::error::$c8s_ns must carry '$label'"
    exit 1
  fi
done
if ! grep -qE '^\s*name:\s*c8s-system\s*$' "$c8s_ns"; then
  echo "::error::$c8s_ns must name the c8s-system namespace"
  exit 1
fi
