...
func TestVerifyCertConfigClaimsPins(t *testing.T) {
	...
	// reject).
	policy := func(opKeys, seed, meshCA []byte) *VerifyPolicy {
		return &VerifyPolicy{AttestationApiURL: "http://127.0.0.1:1", OperatorKeysDigest: opKeys, SeedDigest: seed, MeshCADigest: meshCA}
	}
	...
	t.Run("mesh-ca pin against unset sentinel fails closed", func(t *testing.T) {
