...
| `/etc/kata-opa/default-policy.rego` | this dir | Overlays osbuilder's allow-all with our policy: `SetPolicyRequest` is denied (the host can't swap the policy at runtime), and so are the host-reach-in RPCs `ExecProcessRequest`/`ReadStreamRequest`/`WriteStreamRequest` (no `kubectl exec`/`logs` against a locked guest — see "Debug variant" below) and `GetDiagnosticDataRequest`, whose termination-log branch reads a guest file the request names, and `ReseedRandomDevRequest`, which credits host-chosen entropy to the guest CRNG (both denied in the debug variant too). The agent is built with `AGENT_POLICY=yes` so this is enforced. Per-image-digest gating is policy-monitor's job — see `docs/kata-image-policy.md`. |
...
- **The in-guest `C8S_*` config is a baked default.** There is no per-pod
  ...
  `C8S_CDS_MEASUREMENTS` and the TDX register pins `C8S_CDS_RTMRS`, are
  delivered over init-data instead and checked against the launch-committed
  digest (see
