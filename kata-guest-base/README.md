...
| `/etc/c8s/ghcr-auth.json` | generated by `scripts/fetch.sh` (from `READ_PRIVATE_GHCR_TOKEN`) | docker auth.json for in-guest guest-pull of **private** `ghcr.io/confidential-dot-ai` images. tmpfiles copies it to `/run/image-security/auth.json` at boot — the `file://` path the kata cmdline names. Holds a read-only PAT; part of the measured root. See "Private guest-pull" below. |
...
`ghcr.io/confidential-dot-ai` image that in-guest pull is anonymous by default and
