...
| `/etc/kata-opa/default-policy.rego` | this dir | Overlays osbuilder's allow-all with our policy: allow-all RPCs **except** `default SetPolicyRequest := false`, so the host can't swap the policy at runtime. The agent is built with `AGENT_POLICY=yes` so this is enforced. Per-image-digest gating is policy-monitor's job — see `docs/kata-image-policy.md`. |
...
- **No SSH server.** The image is driven by kata-runtime via vsock +
  cloud-init; rely on `kubectl exec` (kata-agent serves exec over vsock).
