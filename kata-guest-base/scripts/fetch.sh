...
# SHA-256 digests of the c8s container images (cds, get-cert) at
# IMAGE_TAG. The digests are resolved against GHCR via
...
#
# Pre-supplied env vars (set any subset):
#   CDS_DIGEST=sha256:...
#   GET_CERT_DIGEST=sha256:...
#
# If both are set, no oras lookup happens and IMAGE_TAG is
# unused. If either is missing, IMAGE_TAG is required so the
# remaining one can be resolved against the registry.
if [[ -z "${CDS_DIGEST:-}" || -z "${GET_CERT_DIGEST:-}" ]]; then
    if [[ -z "${IMAGE_TAG}" ]]; then
        ...
        echo "         2. Or set CDS_DIGEST and GET_CERT_DIGEST directly" >&2
        echo "            (useful for local builds against images that don't" >&2
        echo "            live in a registry yet)." >&2
        exit 1
    fi
fi
