...
func TestFetchWhitelist_Success(t *testing.T) {
	...
	wl, err := fetchWhitelist(context.Background(), client)
	if err != nil {
...
func TestFetchWhitelist_ServerError(t *testing.T) {
	...
	client := newTestClient(srv.URL)
	_, err := fetchWhitelist(context.Background(), client)
	if err == nil {
...
func TestFetchWithRetries_SuccessOnSecondAttempt(t *testing.T) {
	...
	origFetchTimeout := fetchTimeout
	short := 5 * time.Second
	fetchTimeout = &short
	defer func() { fetchTimeout = origFetchTimeout }()

	client := newTestClient(srv.URL)
	wl, err := fetchWithRetries(context.Background(), client, 3, 10*time.Millisecond)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(wl.Digests) != 1 {
...
func TestFetchWithRetries_AllFail(t *testing.T) {
	...
	client := newTestClient(srv.URL)
	_, err := fetchWithRetries(context.Background(), client, 3, 10*time.Millisecond)
	if err == nil {
