...
// or a -f values file. A -f minTcb that is not a floor string (YAML null, 0,
// false) renders no floor and is an error, not an absent value. An install
// that sets neither keeps the shipped floor; a floor at or above the default
// needs nothing.
...
// flag when set, else the last -f file to mention minTcb (later files win,
...
func effectiveMinTCB(minTCB string, valuesFiles []string) (value, origin string, err error) {
	...
	for _, f := range valuesFiles {
		...
		if v, ok := tree["minTcb"]; ok {
			leaf, origin = v, fmt.Sprintf("values file %q minTcb", f)
		}
	}
	if origin == "" {
		return "", "", nil
	}
	value, err = minTcbFileLeaf(leaf)
	if err != nil {
		return "", "", fmt.Errorf("%s: %w", origin, err)
	}
...
// minTcbFileLeaf resolves a -f file's minTcb leaf. Only a string renders a
// floor: a YAML null deletes the key at helm's values coalesce and 0/false
// are falsy under the chart's guards, so a present non-string renders no
// floor and fails the chart render — error here rather than treat the file
// as not setting minTcb. The deliberate floorless spelling is "".
func minTcbFileLeaf(leaf any) (string, error) {
	if s, ok := leaf.(string); ok {
		return s, nil
	}
	return "", fmt.Errorf("must be a floor string like \"3,0,8,0\", got %v — YAML null/0/false render no floor; spell the deliberate floorless value minTcb: \"\"", leaf)
}
...
func init() {
	...
	installCmd.Flags().BoolVar(&installForce, "force", false, "proceed past guarded prompts — currently: install without --operator-keys (allowlist writes disabled); install with a minTcb (flag or -f values file) below the chart's shipped TCB floor. A values-file minTcb that is not a floor string (YAML null, 0, false) is an error, not a downgrade --force bypasses; the deliberate floorless value is minTcb: \"\"")
