...
var renderValuesCmd = &cobra.Command{
	...
	Long: `Computes the same Helm values as 'c8s install' — resolved image digests,
--cvm-mode TEE devices, --single-node node selector, --kata toggles, and the
NRI allowlist derivation — and writes them to stdout as a values.yaml, without
...
Requires the 'helm' CLI on PATH, and 'crane' unless --resolve-digests=false.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		setArgs, err := buildValueArgs(cmd.Context(), cmd, components, resolveImageTag(), distro)
		if err != nil {
			return err
		}
		...
		fmt.Fprintf(os.Stdout, "# Generated by `c8s render-values` (chart %s).\n# Do not hand-edit; re-run render-values to refresh.\n", version.Version)
		_, err = os.Stdout.Write(out)
		return err
	},
}

// resolveDigestArgs is the digest-resolution step of buildValueArgs, indirected
// so tests can substitute a stub for the crane-shelling appendResolvedDigestArgs
// and exercise the full builder offline.
var resolveDigestArgs = appendResolvedDigestArgs
...
// never touches a cluster. The returned slice is value args only — install
// prepends the `upgrade --install <release> <chart>` verb and appends -f / wait
// flags, render-values converts these to a values tree.
func buildValueArgs(ctx context.Context, cmd *cobra.Command, components []c8sComponent, imageTag, distro string) ([]string, error) {
	var setArgs []string
	...
	if installResolveDigests {
		var err error
		setArgs, err = resolveDigestArgs(ctx, setArgs, imageTag, components)
		if err != nil {
			return nil, err
		}
	}
...
// replaced with a fresh map (`X=null` then `X.y=v` yields {X: {y: v}}).
