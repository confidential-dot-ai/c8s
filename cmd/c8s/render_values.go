...
func init() {
	...
	renderValuesCmd.Flags().StringVar(&installCvmMode, flagCvmMode, "", "CVM deployment shape (REQUIRED; orthogonal to --hardware-platform): pod (per-pod kata CVMs; disables host-side ratls-mesh/attestation-api/nri-image-policy) or node (generalized node-as-CVM native TEE device) or gke (GKE managed CVMs) or aks (vTPM /dev/tpm0). node/gke/aks are single-tenant — the node is one trust domain; only pod isolates workloads from each other")
	...
	renderValuesCmd.Flags().StringSliceVar(&installPCRs, "pcrs", nil, "Azure vTPM PCR pin(s) <index>=<sha256-hex> completing --measurements on --cvm-mode=aks (repeatable/comma-separated). Emits cds.pcrs + ratlsMesh.pcrs; ignored for non-vTPM evidence")
	renderValuesCmd.Flags().StringVar(&installInitDataHash, "init-data-hash", "", "hex SHA-256 init-data digest the cluster's evidence must bind (vTPM PCR[8] on az). Emits cds.initDataHash + ratlsMesh.initDataHash")
