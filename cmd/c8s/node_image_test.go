...
func TestNodeImageRender(t *testing.T) {
	...
	for _, platform := range []string{"tdx", "sev-snp"} {
		t.Run(platform, func(t *testing.T) {
			...
			for _, directive := range []string{"listen 443 ssl;", "server_name c8s-node.invalid;", "pid /run/nginx.pid;", "error_log stderr warn;", "access_log syslog:server=unix:/dev/log,tag=c8s_nginx,nohostname main;", "/run/c8s-tls/cert.pem", "/run/c8s-tls/key.pem", "/run/c8s-tls/ca.pem", "/run/c8s-tls/discovery.json", "location = /allowlist", "limit_req zone=allowlist_write", "127.0.0.1:8801", "127.0.0.1:8800", "location /healthz"} {
