...
func TestRotateTokenSigner_NewKeypair(t *testing.T) {
	ctx := context.Background()
	logger := slog.Default()

	// Create existing token-signer Secret.
	oldKey, _ := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	oldCertPEM := selfSignedCertPEM(t, oldKey, "Old Token Signer", false)
	oldKeyPEM := ecKeyPEM(t, oldKey)

	client := fake.NewSimpleClientset(&corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "kbs-token-signing-keys",
			Namespace: "tee-attestation",
		},
		Data: map[string][]byte{
			"token-signer.key": oldKeyPEM,
			"token-signer.crt": oldCertPEM,
		},
	})

	err := rotateTokenSigner(ctx, client, "tee-attestation", 365, logger)
	if err != nil {
		t.Fatalf("rotateTokenSigner failed: %v", err)
	}

	// Verify Secret was updated.
	secret, err := client.CoreV1().Secrets("tee-attestation").Get(ctx, "kbs-token-signing-keys", metav1.GetOptions{})
	if err != nil {
		t.Fatal(err)
	}

	newCertPEM := secret.Data["token-signer.crt"]
	newKeyPEM := secret.Data["token-signer.key"]

	if string(newCertPEM) == string(oldCertPEM) {
		t.Error("certificate was not rotated")
	}
	if string(newKeyPEM) == string(oldKeyPEM) {
		t.Error("key was not rotated")
	}

	// Verify new cert is self-signed P-256.
	block, _ := pem.Decode(newCertPEM)
	if block == nil {
		t.Fatal("no PEM block in new cert")
	}
	cert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		t.Fatalf("parse new cert: %v", err)
	}
	if cert.Subject.CommonName != "KBS Token Signer" {
		t.Errorf("CN = %q, want %q", cert.Subject.CommonName, "KBS Token Signer")
	}
	ecPub, ok := cert.PublicKey.(*ecdsa.PublicKey)
	if !ok {
		t.Fatal("new cert public key is not ECDSA")
	}
...
func TestRotateTokenSigner_SecretNotFound(t *testing.T) {
	ctx := context.Background()
	logger := slog.Default()

	// Empty clientset — no Secret exists.
