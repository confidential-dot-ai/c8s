...
2. Host iptables NAT PREROUTING DNATs pod-to-pod TCP to this node's `nodeIP:15001` (outbound listener). OUTPUT REDIRECT covers host-originated traffic to pod IPs. routed CNIs counts a PREROUTING REDIRECT hit but never completes the redirected TCP connect; DNAT to the node-local listener follows the same path pods can reach directly.
...
`NET_ADMIN` iptables rules in the host NAT PREROUTING chain DNAT pod-to-pod TCP traffic to this node's outbound listener (`nodeIP:15001`) when the source is a local pod IP and the destination is any known pod IP. OUTPUT REDIRECT rules cover host-originated traffic to pod IPs, where loop prevention via UID exclusion keeps mesh process traffic (UID 1337) out of the interception path. PREROUTING uses DNAT rather than REDIRECT because routed CNIs counts PREROUTING REDIRECT hits but does not complete the redirected pod TCP connect; DNAT to the node-local listener follows the same path pods can reach directly. ClusterIP Services, metadata, kube API, public egress, and direct external-to-pod traffic fall through because they are not local pod-to-pod egress flows. The outbound listener also rejects original destinations that are not known pod IPs, so direct connections to the host-network listener do not become plaintext proxy sessions.

The fall-through flows would reach pods in plaintext, which is unacceptable for confidential workloads specifically: a ClusterIP Service selecting cw pods lets kube-proxy DNAT VIP traffic straight to the pod. The always-on cw guard installs a filter-table chain (`RATLS-MESH-CW`, jumped from FORWARD position 1) that drops all-protocol traffic to an ipset of `confidential.ai/cw`-labeled pod IPs; a conntrack ESTABLISHED,RELATED RETURN passes replies to cw-pod egress. Because some dataplanes (e.g. Cilium) do not track a reply as ESTABLISHED on FORWARD, the chain also RETURNs replies from a source-port allowlist (`--cw-inbound-passthrough`, default `udp:53,tcp:53` so DNS resolves and get-cert works) ahead of the drop. An exemption is bounded to reply traffic: the destination port must fall in the stock ephemeral window (32768-60999), and a TCP exemption additionally matches only the handshake reply and segments with SYN clear, so no TCP exemption can open a connection to a cw pod's own listening ports. UDP has no equivalent gate, so a UDP exemption still admits any datagram from the allowlisted source port into that window. An empty list is the strict drop-all posture. Legitimate delivery never traverses FORWARD (mesh delivery and kubelet probes are host OUTPUT; meshed pod egress is DNAT'd to INPUT in PREROUTING), so the guard turns every plaintext bypass into a visible drop without touching the happy path. See the README "Confidential-workload inbound guard" section for the defended/not-defended list.
...
| Node-level TEE boundary | Node CVMs run SNP or TDX at the VM level. All pods on a node share the same TEE. Per-pod isolation uses Kata on native hardware. | Can't distinguish between pods on the same node at the attestation level. |
...
| UID-based iptables exclusion | Mesh process (UID 1337) must be exempt from redirect to prevent infinite loops. UID matching is the simplest Linux mechanism for this (host-side). | Fixed UID (must not collide with other processes). |
| cgroup-scoped in-guest exemptions | In-guest, the mesh proxy's and attestation-service's egress are exempted by systemd-slice cgroup path, not by UID — `--uid-owner` cannot tell a root workload from attestation-service. | Requires exact kata-guest slice paths; unverified on a live guest image. |
...
| `preStop` cleanup + sleep | Runs `iptables-cleanup --keep-guard` before termination: the traffic-interception NAT rules are removed while the fail-closed filter guard stays live — unmeshed inbound and non-TCP egress are still dropped; TCP to non-pod destinations was never meshed. The 5s sleep gives K8s time to update endpoints after cleanup. | Adds 5s to every pod termination. |
...
`iptables-cleanup --keep-guard` (what the chart's preStop hook runs) removes the traffic-interception pieces — the OUTPUT and PREROUTING jumps, the nat `RATLS-MESH`/`RATLS-MESH-PREROUTING` chains, and the pod/local-pod ipsets — but **keeps the fail-closed guard**: the filter `RATLS-MESH-CW` and `RATLS-MESH-CW-EGRESS` chains, their FORWARD jumps, and the cw pod ipsets. Under the guard, unmeshed inbound and non-TCP egress are still dropped; TCP to non-pod destinations was never meshed. A full `iptables-cleanup` (no flag) additionally removes the guard. The preStop hook runs cleanup followed by a 5s sleep (separated by newline, not `&&`, so sleep always runs regardless of cleanup exit code).
...
During the window the fail-closed guard stays live (`--keep-guard` in preStop keeps the filter chains and their ipsets installed): unmeshed inbound to a cw pod (FORWARD `RATLS-MESH-CW` drop) and non-TCP egress from one (`RATLS-MESH-CW-EGRESS` drop) are still dropped. The window therefore only affects pod-to-pod TCP that was previously DNAT-redirected, which is the mesh's soft-fail boundary for TCP — the same posture the guard has always had, now exercised over the restart boundary.
...
without a host-local pod CIDR, with fabric-routable pod IPs. The
...
| `ratls_mesh_iptables_cw_inbound_drops_total` | gauge | — | Packets dropped by the cw guard chain: non-mesh traffic to a confidential-workload pod. Also increments where replies depend on the exemption (untracked reply tuple) and a cw pod's `ip_local_port_range` sits outside the exemption window |
| `ratls_mesh_iptables_cw_passthrough_returns_total` | gauge | — | Packets admitted by a cw guard passthrough exemption: reply traffic from an allowlisted source port that the dataplane did not track as `ESTABLISHED` |
