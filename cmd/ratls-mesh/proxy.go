...
type Proxy struct {
	...
	srcConns       sync.Map      // srcIP (string) → *atomic.Int64
	onReady        func()
	onShutdown     func()
...
type accessLogEntry struct {
	...
	certMode     string        // "self-signed" or "kbs"
...
// srcConnCounter returns the atomic counter for a source IP, creating it if needed.
func (p *Proxy) srcConnCounter(srcIP string) *atomic.Int64 {
	if v, ok := p.srcConns.Load(srcIP); ok {
		return v.(*atomic.Int64)
	}
	v, _ := p.srcConns.LoadOrStore(srcIP, &atomic.Int64{})
	return v.(*atomic.Int64)
}
